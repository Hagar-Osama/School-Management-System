...
@section('css')
    @toastr_css
@section('title')
{{trans('payments.Add Payments')}} {{$student->name}}
@stop
@endsection
@section('page-header')
<!-- breadcrumb -->
<div class="page-title">
    <div class="row">
        <div class="col-sm-6">
            <h4 class="mb-0"> {{trans('payments.Add Payments')}} : <label style="color: red">{{$student->name}}</label></h4>
        </div>
        <div class="col-sm-6">
            <ol class="breadcrumb pt-0 pr-0 float-left float-sm-right ">
                <li class="breadcrumb-item"><a href="{{route('dashboard')}}" class="default-color">{{trans('classes.Home')}}</a></li>
                <li class="breadcrumb-item active">{{trans('payments.Add Payments')}}</li>
            </ol>
        </div>
    </div>
</div>
